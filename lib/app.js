...
const { skipTodayIfPast } = require('./scheduler');
...
function createApp({ config, store, tuya, auth, logger = console }) {
  ...
  api.post('/automations', wrap(async (req, res) => {
    ...
    const automation = await store.mutate((s) => {
      ...
      skipTodayIfPast(created, new Date(), config.timeZone);
  ...
  api.patch('/automations/:id', wrap(async (req, res) => {
    ...
    const automation = await store.mutate((s) => {
      ...
      if (timeChanged || daysChanged) skipTodayIfPast(existing, new Date(), config.timeZone);
      return existing;
    });
